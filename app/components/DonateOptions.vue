  ...
  <ul :class="compact ? 'sm:grid-cols-3' : 'sm:grid-cols-2 lg:grid-cols-3'" class="grid gap-3">
    <li v-for="method in DONATION_METHODS" :key="method.key">
      <UiCard
        ...
        class="group flex h-full items-center gap-3 hover:border-primary-300 dark:hover:border-primary-400/40"
      >
        <Icon
          :name="method.icon"
 ...
 *
 * Each option is a `UiCard` rendered as an anchor (`tag="a"`), so the whole tile
 * is the link. Every link is external, so each carries `rel="noopener
 * noreferrer"` and a screen-reader-only note that it opens a new tab. The
 * platform names are not translated; only the "monthly or one-off" caption is.
 *
